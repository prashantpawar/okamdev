...
/**
 * MODULE NAME   : Ok_dnas.php
 *
 * DESCRIPTION   : Ok_dnas module controller
 *
 * MODIFICATION HISTORY
 *   V1.0   2009-11-28 10:26 PM   - Pradesh Chanderpaul     - Created
 *
 * @package             Ok_dnas
 * @subpackage          ok_dnas component Class
...
class Ok_dnas extends Controller {
   ...
   */
   ...
   // Function: showall()
   //
   // Description: Extracts a list of all ok_dnas data records and displays it.
   //
   // //////////////////////////////////////////////////////////////////////////
   function browse() {
      ...
      // ...the ok_dnas model and sending it a 'retrievelist' request.
      // ///////////////////////////////////////////////////////////////////////
      ...
      $this->load->model('ok_dnasmodel');                  // Instantiate the model
      $the_results['ok_dnas_list'] = $this->ok_dnasmodel->findAll($start, $limit_per_page);  // Send the retrievelist msg
      // $the_results['rowcount'] = count($the_results['ok_dnas_list']);
      ...
      $config['base_url']     = site_url('ok_dnas/showall/');   // or just /ok_dnas/
      $config['total_rows']   = $this->ok_dnasmodel->table_record_count;
      ...
      $this->layout->render_page('/ok_dnas/ok_dnasgrid', $the_results);
      // NOTE: If you don't want to use the layout library, use the line below.
      // $this->load->view('/ok_dnas/ok_dnasgrid', $the_results);
   ...
   // Function: add()
   //
   // Description: Prompts user for input and adds a new ok_dnas entry
   //              ...onto the database.
   //
   // //////////////////////////////////////////////////////////////////////////
   function add() {
      ...
      if ( $submit != false ) {
		...
		$data['answer_id']		= $this->input->post('answer_id', TRUE);
		$data['relevance_id']		= $this->input->post('relevance_id', TRUE);
         ...
         */
         ...
         $this->ok_dnasmodel->add($data);

         // $this->load->helper('url');
         redirect('/ok_dnas/', 'location');
		...
		// Retrieve the ok_relevancies lookup values.
		$this->load->model('ok_relevanciesmodel');
		$data['ok_relevancieslist'] = $this->ok_relevanciesmodel->findAll();
		...
		$data['ok_answerslist'] = $this->ok_answersmodel->findAll();
         ...
         $this->load->library('layout');
         $this->layout->render_page('/ok_dnas/ok_dnasdetails', $data);
         // NOTE: If you don't want to use the layout library, use the line below.
         // $this->load->view('/ok_dnas/ok_dnasdetails', $data);
   ...
   function modify() {
      ...
      if ( $submit != false ) {
         ...
         */
         ...
         $this->ok_dnasmodel->modify($data['id'], $data);
         ...
         $this->load->model('ok_dnasmodel');
         $data = $this->ok_dnasmodel->retrieve_by_pkey($idField);
         $data['action'] = 'modify';
   ...
   function delete() {
      ...
      $this->load->model('ok_dnasmodel');
      $the_results = $this->ok_dnasmodel->delete_by_pkey($idField);
