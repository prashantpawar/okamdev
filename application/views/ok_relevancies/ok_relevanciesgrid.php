<?
   ...
   $modify_url = site_url('ok_relevancies/modify/');
   $delete_url = site_url('ok_relevancies/delete/');
   $add_url    = site_url('ok_relevancies/add/');
...
<form action='<?= $add_url; ?>' method='POST' id='frmok_relevancies'>
<table width="100%" border="0" cellspacing="1" cellpadding="0">
<tr>
   <th align="left" class="formSecHeader">Browsing ok_relevancies values</th>
   <th align="right">
...
<tbody  class="scrollingContent">
      ...
      <?
         $i = 0;
         foreach ($ok_relevancies_list as $ok_relevancies) {
            $i++;
            if (($i%2)==0) { $bgColor = "#FFFFFF"; } else { $bgColor = "#C0C0C0"; }
      ?>
      <tr bgcolor="<?= $bgColor; ?>">
         <td align="center" nowrap="nowrap">
            <a href = "<?= $modify_url."/".$ok_relevancies["id"]; ?>" >Edit</a>
            &nbsp;&nbsp;&nbsp;
            <a href = "<?= $delete_url."/".$ok_relevancies["id"]; ?>" >Delete</a>
         </td>
   <td align="left" nowrap="nowrap"><?= $ok_relevancies['id']; ?></td>
   <td align="left" nowrap="nowrap"><?= $ok_relevancies['relevancy']; ?></td>
</tr>
      <? } ?>
</tbody>
</table>
